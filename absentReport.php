...
<a class="btn btn-primary mx-2" href="/gujratSports/absentReport.php">ABSENT REPORT<span class="sr-only">(current)</span></a>
...
?>
 <form action="absentReport.php" method="GET">
...
$too = date('h:i');
  echo '<h4 align="center">Absent Report of Date :- '.$dy.' / '.$mnth.' / '.$yr.'</h4>';
...
while($row = mysqli_fetch_assoc($result))
{
    ...
    $name = $row['FIRSTNAME'];
     ...
     $sql1 = "SELECT * FROM `attendence` where `day` = '$dy' AND `month` = '$mnth' AND `year` = '$yr' AND `$userid` = '0'";
